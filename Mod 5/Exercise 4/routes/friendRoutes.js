...
router.get('/filter', (req, res) => {
    ...
    if (matchingFriends.length > 0) {
        ...
        // and an error response when there are no matches
        res.status(404).json({error: "No friends matching gender "+filterLetter})
    }  
})
...
router.get('/:id', (req, res) => {
    ...
    let friend = friends.find(user => friend.id == friendId)
    friend ? res.status(200).json({result:user})
...
router.put('/:id', (req, res) => {
    ...
    // Find the existing friend by ID
    let friendIndex = users.findIndex(friend => friend.id === friendId);

    // If the friend is not found, return a 404
    if (friendIndex === -1) {
        res.status(404).json({ error: 'Friend not found' });
        return;
    }

    // Validate that the updated friend contains a name and gender
    ...
    users[friendIndex] = { ...users[friendIndex], ...updatedFriend };

    // Return the updated friend
    res.status(200).json(users[friendIndex]);
});
